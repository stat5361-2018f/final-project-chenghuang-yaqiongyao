...
If we set this equation to be $u$, which is probablity of $x \le X$. We can hardly have the close form solution of $x$. Thus, if the support of $x$ is not $\mathcal{R}$, then no close form can be obtained. 
<!--
If we select triangular kernel, the CDF is:
$$F_K(x|\mathcal{D}) = \frac{1}{n}\left(\sum_{i=1}^n\frac{(b -x_i+x)^2}{2b^2} I(x_i-b < x< x_i) +\sum_{i=1}^n ( 1 - \frac{(b -x + x_i)^2}{2b^2}) I(x_i < x< x_i + b) + \sum_{i=1}^n I(x \ge x_i +  b) \right).$$
-->

Meanwhile, if we select more complicated kernels, such as normal distribution, the CDF of KDE becomes:
...
pdf('p1.pdf')
plot(x, y, main = "generated copulas x ~ Gamma(1,2) y ~ chisq(10)")
dev.off()
```
...
plot(x, y, xlim = c(0, 15), ylim = c(0, 35))
pdf('p2.pdf')
plot(xy_kde[,1], xy_kde[,2], xlim = c(0, 12), ylim = c(0, 30), main = "Generate sample using Kernel Density Estimate(KDE), n = 1000", xlab = "x", ylab = "y")
dev.off()
```

# Reference
http://home.cc.umanitoba.ca/~farhadi/ASPER/Kernel%20Smoothing.pdf \
